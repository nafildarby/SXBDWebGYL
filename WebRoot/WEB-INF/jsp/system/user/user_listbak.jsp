			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th>上次登录IP</th>
						 -->
						<th><i class="icon-envelope"></i>电话</th>
					</tr>
				</thead>
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								<td class='center' style="width: 30px;">
									<c:if test="${user.UserNo != 'admin'}"><label><input type='checkbox' name='ids' value="${user.Id }" id="${user.Email }" alt="${user.Telphone }"/><span class="lbl"></span></label></c:if>
									<c:if test="${user.UserNo == 'admin'}"><label><input type='checkbox' disabled="disabled" /><span class="lbl"></span></label></c:if>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td>${user.Id }</td>
								<td><a>${user.UserNo }</a></td>
								<td>${user.UserName }</td>
								<td>${user.ROLENAME }</td>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<a class='btn btn-mini btn-warning' title="发送短信" onclick="sendSms('${user.Telphone }');"><i class='icon-envelope'></i></a>
										</c:if>
										
										<c:if test="${QX.edit == 1 }">
											<c:if test="${user.UserNo != 'admin'}"><a class='btn btn-mini btn-info' title="编辑" onclick="editUser('${user.Id }');"><i class='icon-edit'></i></a></c:if>
											<c:if test="${user.UserNo == 'admin'}"><a class='btn btn-mini btn-info' title="您不能编辑"><i class='icon-edit'></i></a></c:if>
										</c:if>
										<c:choose>
											<c:when test="${user.UserNo=='admin'}">
												<a class='btn btn-mini btn-danger' title="不能删除"><i class='icon-trash'></i></a>
											</c:when>
											<c:otherwise>
												<c:if test="${QX.del == 1 }">
												 <a class='btn btn-mini btn-danger' title="删除" onclick="delUser('${user.Id }','${user.UserNo }');"><i class='icon-trash'></i></a>
												</c:if>
											</c:otherwise>
										</c:choose>
									</div>
