       ...
       <div title="明细信息" style="padding:10px;">
            ...
            <table class="tab" border="0" cellpadding="3px" cellspacing="0" width="100%">
             	<tbody>  
                   <tr>  
                        ...
                        <td>
                        	<input id="outCode" class="easyui-textbox" value="${pd.AllocationNo}"  style="width:180px;" />
                        </td>
                    ...
                    <tr> 
                        ...
                        <td>
                        	<input id="OutTime" class="easyui-textbox" value="${pd.AllocationTime}"  style="width:180px;" />
                        </td>
                        <td class="tabAlign">操作人：</td>
                        <td>
                        	<input id="OpName" class="easyui-textbox" value="${pd.AllocationPerson}"   style="width:180px;" />
                        </td> 
                    </tr> 
                    <tr>
        ...
        <div style="margin:0px 10px 0px 10px;clear:both;">
         	<fieldset class="scheduler-border">
                ...
                <div style="width: auto; background-color: rgb(222, 235, 250); margin: 2px;" class="panel datagrid">
					<div  title="" class="datagrid-wrap panel-body panel-body-noheader"> 
						<table id="allot" data-options="iconCls:'icon-edit'">
					        <thead>
					            <tr>  
					                <th data-options="field:'ProductName',align:'center',width:fixWidth(0.20)">物料名称</th>
					                <th data-options="field:'BarCode',align:'center',width:fixWidth(0.20)">物料编码</th>
					                <th data-options="field:'Comment',align:'center',width:fixWidth(0.20)">物料描述</th>
					                <th data-options="field:'TotalNum',align:'center',width:fixWidth(0.14)" >调配数量</th>
					                <th data-options="field:'Unit',align:'center',width:fixWidth(0.145)">单位</th> 
					            </tr>
					        </thead>
					        <tbody>
...
<script type="text/javascript"> 
	...
	function appliBtn(){ 
		$.ajax({
			type:"post",
			url:"allot/saveApplyData",
			dataType:"json",
			data:{ 
				"ApplyNo": '${pd.AllocationNo}',
				"Common":$("#common").val()
			}, 
			success:function(data){ 
				if(data.msg == "failed"){
					...
					$("#SelectallotWindow").window('close');
				};
			},
			error:function(){ 
				alert("审核失败"); 
	 ...
	 function repulseBtn() {  
		$.ajax({
			type:"post",
			url:"allot/saveApplyDataRepulse",
			dataType:"json",
			data:{ 
				"ApplyNo": '${pd.AllocationNo}',
				"Common":$("#common").val()
