       ...
       <div title="明细信息" style="padding:10px;">
            ...
            <table class="tab" border="0" cellpadding="3px" cellspacing="0" width="100%">
             	<tbody> 
                     <tr>  
                        ...
                        <td>
                        	<input id="outCode" class="easyui-textbox" value="${pd.ScrappedNo}"  style="width:240px;" />
                        </td>
                        <td class="tabAlign">报损仓库：</td>
                        <td>
                        	<input id="outWHNo"  class="easyui-textbox" value="${pd.WarehouseName}"  style="width:240px;" />
                        </td>
                    </tr>
                    <tr>  
                        <td class="tabAlign">报损时间：</td> 
                        <td>
                        	<input id="OutTime" class="easyui-textbox" value="${pd.ScrappedTime}"  style="width:240px;" />
                        </td>
                        <td class="tabAlign">操作人：</td>
                        <td>
                        	<input id="OpName" class="easyui-textbox" value="${pd.ScrappedPerson}"   style="width:240px;" />
                        </td> 
                    </tr> 
                    <tr>
        ...
        <div style="margin:0px 10px 0px 10px;clear:both;">
         	<fieldset class="scheduler-border">
                ...
                <div style="width: auto; background-color: rgb(222, 235, 250); margin: 2px;" class="panel datagrid">
					<div  title="" class="datagrid-wrap panel-body panel-body-noheader"> 
						<table id="scrap" data-options="iconCls:'icon-edit',singleSelect:true">
					        <thead>
					            <tr>  
					            	<th data-options="field:'ProductName',align:'center',width:fixWidth(0.24)">物料名称</th>
					                <th data-options="field:'BarCode',align:'center',width:fixWidth(0.24)">物料条码</th> 
					                <th data-options="field:'Number',align:'center',width:fixWidth(0.175)" >报损数量</th> 
					                <th data-options="field:'Comment',align:'center',width:fixWidth(0.23),editor:'textbox'">备注</th>  
					            </tr>
					        </thead>
					        <tbody>
...
<script type="text/javascript"> 
	...
	function appliBtn(){ 
		$.ajax({
			type:"post",
			url:"scrap/saveApplyData",
			dataType:"json",
			data:{ 
				"ApplyNo":'${pd.ScrappedNo}',  
				"Common":$("#Note").val()   
			}, 
			success:function(data){ 
				if(data.msg == "failed"){
					...
					$("#SelectscrappedNoWindow").window('close');					
				};
			},
			error:function(){ 
				alert("审核失败"); 
	...
	function repulseBtn() {  
		$.ajax({
			type:"post",
			url:"scrap/saveApplyDataRepulse",
			dataType:"json",
			data:{ 
				"ApplyNo": '${pd.ScrappedNo}',
				"Common":$("#Note").val()   
