		...
		if(ivk.getTarget() instanceof RoutingStatementHandler){
			...
			if(mappedStatement.getId().matches(pageSqlId)){ //拦截需要分页的SQL
				...
				if(parameterObject==null){
					...
					List<ParameterMapping> temp = boundSql.getParameterMappings();
					
					//针对SQLSERVER的拼接语句，进行?的二次赋值
					if(!temp.isEmpty())
						temp.addAll(temp);
				}
			}
	...
	private String generatePageSql(String sql,Page page){
		if(page!=null && Tools.notEmpty(dialect)){
			...
			if("mysql".equals(dialect)){
				...
				pageSql.append(page.getpage()*page.getrows() > page.getTotalResult()?page.getTotalResult()%page.getrows():page.getrows());
				pageSql.append(" * FROM (");
				pageSql.append(sql);
				pageSql.append(") AS TEMP WHERE ID IN ( SELECT TOP ");
				pageSql.append(page.getpage() * page.getrows());
				pageSql.append(" ID FROM (");
				pageSql.append(sql);
				pageSql.append(" ) AS TEMP ");
				if(sql.lastIndexOf("ORDER BY") > 0)
					pageSql.append(sql.substring(sql.lastIndexOf("ORDER BY")));
				pageSql.append(" ) ORDER BY TEMP.ID) AS TEMP2 ORDER BY TEMP2.ID DESC");
			}
