...
public class LoginController extends BaseController {
		...
		if (null != KEYDATA && KEYDATA.length == 3) {
			...
			if (null == code || "".equals(code)) {
				...
				if (Tools.notEmpty(sessionCode)
					...
					String passwd = MD5.md5(PASSWORD, "UTF-16LE");
					...
					if (pd != null) {
						...
						User user = new User();
						user.setUSER_ID(pd.get("Id").toString());
						user.setUSERNAME(pd.getString("UserNo"));
						user.setPASSWORD(pd.getString("Password"));
						user.setNAME(pd.getString("UserName"));
						user.setRIGHTS(pd.getString("Rights"));
						user.setROLE_ID(pd.get("RoleId").toString());
				...
				if (null == session.getAttribute(Const.SESSION_allmenuList)) {
					...
					if (Tools.notEmpty(roleRights)) {
						for (Menu menu : allmenuList) {
							menu.setHasMenu(user.getUSERNAME().equals("admin") ? true : RightsHelper.testRights(roleRights,menu.getMENU_ID()));
							if (menu.isHasMenu()) {
								List<Menu> subMenuList = menu.getSubMenu();
								for (Menu sub : subMenuList) {
									sub.setHasMenu(user.getUSERNAME().equals("admin") ? true : RightsHelper.testRights(roleRights, sub.getMENU_ID()));
								}
							}
						}
					}
	...
	public Map<String, String> getUQX(Session session) {
		...
		try {
			...
			String ROLE_ID = userService.findByUId(pd).get("RoleId").toString();
